  ...
  switch (action.type) {
    case 'todo_new/ADD_TODO':
      ...
      return {
        ...
        todoTitles: [
          ...state.todoTitles,
          { todoTitle: 'todo without name', id: `todo${state.idGen}` },
        ],
        todos: { ...state.todos, [`todo${state.idGen}`]: [] },
      }
    case 'todo_new/DELETE_TODO':
      let newTodoTitles = state.todoTitles.filter(
      ...
      for (let key in newTodos) {
        key === action.noteId && delete newTodos[key]
      }
      return {
        ...state,
...
export const actions = {
  ...
  deleteTodo: (noteId: string) =>
    ({ type: 'todo_new/DELETE_TODO', noteId } as const),
  addTodoItem: (noteId: string, value: string, importance: string) =>
    ({ type: 'todo_new/ADD_TODO_ITEM', noteId, value, importance } as const),
  deleteTodoItem: (noteId: string, itemId: number) =>
    ({ type: 'todo_new/DELETE_TODO_ITEM', noteId, itemId } as const),
  selectTodo: (noteId: string) =>
    ({ type: 'todo_new/SELECT_TODO', noteId } as const),
  modifyTodoTitle: (noteId: string, title: string) =>
    ({ type: 'todo_new/CHANGE_TODO_TITLE', noteId, title } as const),
  ...
  changeTodoItemContent: (noteId: string, content: Array<ContentItemType>) =>
    ({ type: 'todo_new/CHANGE_TODO_ITEM_CONTENT', noteId, content } as const),
}
